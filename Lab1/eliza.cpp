...
std::vector <std::pair <std::string, std::string>> switchViewPointRules =
{
	{
		"I",
		"you"	
...
main(int argc, char const *argv[])
{
	...
	while (1)
	{
				...
				if (special != std::string::npos && (special > 0 && special != (rule.first.size() - 1)))
				{
					...
					if (f1 >= 0 && f2 >= 0)
					{
						...
						if (it)
						{
							...
							for (auto rsvp : switchViewPointRules)
							{
				...
				else
				{
					...
					if (found >= 0)
					{
						...
						if (it2 != std::string::npos)
						{
							...
							for (auto rsvp2 : switchViewPointRules)
							{
		...
		for (std::string::size_type i = 0; i < output.length(); ++i)
    		std::cout << std::toupper(output[i], loc);
